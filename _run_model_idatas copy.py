...
def worker(task):
    #print(task, ' idata')
    (f, sigma, implementation, penalised, replication, builder, first_sample) = task
    if first_sample:
        print("first sample")
...
tasks_df['first_sample'] = False
tasks_df_first = tasks_df[-idatas_workers:].copy()
tasks_df_first['first_sample'] = True
tasks_df = pd.concat([tasks_df_first, tasks_df]).reset_index(drop=True)
