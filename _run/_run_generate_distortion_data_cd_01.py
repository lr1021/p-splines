...
def generate_data_dict(distribution_list, n_list, lim_list, sparse_list, replication_list, x_range=[0.0, 1.0], seed=0):
    data_dict = {}
            ...
            for lim in lim_list:
                ...
                for sparse in sparse_list:
                    for i, r in enumerate(replication_list):
                        num_elements = np.sum(mask[i])
                        keep = np.random.choice(np.arange(1, num_elements), size=int(sparse * num_elements)-1, replace=False)
                        keep = np.concatenate([[0], keep])
                        x_n_r = x_n[i].copy()
                        x_n_r = np.concatenate([x_n_r[mask[i]][keep], x_n_r[~mask[i]]])
                        x_n_r.sort()
                        data_dict[(dist, n_val, lim, sparse, r)] = x_n_r
    return data_dict

def generate_curve_dict(distribution_list, n_list, lim_list, sparse_list, replication_list, num_knots, data_dict, plot=False):
    degree = 3
    curve_dict = {}
    pen_dict = {}
    curve_plot_dict = {}
                    ...
                    for r in replication_list:
                        ...
                        if len(x_vals)>num_knots:
                            ...
                            if plot:
                                ...
                                curve_plot_dict[(dist, n_val, lim, sparse, r)] = curve_plot
                            curve = (curve - np.min(curve))/(np.max(curve) - np.min(curve))
                            curve_dict[(dist, n_val, lim, sparse, r)] = curve
                            pen_dict[(dist, n_val, lim, sparse, r)] = pen
    if plot:           
        return curve_dict, curve_plot_dict, pen_dict
    else:
        return curve_dict, pen_dict
    
def generate_data(scale_list, distribution_list, n_list, lim_list, sparse_list, sigma_list, replication_list, num_knots, data_dict, curve_dict):
    f_sigma_list = []
    for dist in distribution_list:
        for n_val in n_list:
            for lim in lim_list:
                for sparse in sparse_list:
                    for scale in scale_list:
                        for sigma in sigma_list:  
                            f_sigma_list.append((f'{dist}_{num_knots}_{n_val}_{lim}_{sparse}_{scale}', sigma))
            ...
            for lim in lim_list:
                for sparse in sparse_list:
                    for i, r in enumerate(replication_list):
                        if r % 20 == 0:
                            print(r)
                        x_n_r = data_dict[(dist, n_val, lim, sparse, r)].copy()
                        eps_n_r = eps[i, :len(x_n_r)].copy()
                        
                        curve = curve_dict[(dist, n_val, lim, sparse, r)].copy()
                        curve = curve.flatten()
                        
                        for scale in scale_list:
                            for sigma in sigma_list:
                                y = (curve + sigma * eps_n_r/5.0) * scale
                                data[(f'{dist}_{num_knots}_{n_val}_{lim}_{sparse}_{scale}', sigma)][r] = (x_n_r, y)
    return data
...
num_knots = 5
lim_list = [num_knots / (num_knots + 1)]
sparse_list = [0.05, 0.1, 0.2]
n_replications = 100
replication_list = list(range(n_replications))
...
data_dict_path = os.path.join("../data", "data_dictCD20_01.pkl")
if gen_data_dict:
    data_dict = generate_data_dict(distribution_list, n_list, lim_list, sparse_list, replication_list)
...
curve_dict_path = os.path.join("../data", f"curve_dict_CD{num_knots}_01.pkl")
...
if gen_curve_dict:
    curve_dict, curve_plot_dict, pen_dict = generate_curve_dict(distribution_list, n_list, lim_list, sparse_list, replication_list, num_knots, data_dict, plot=True)
...
data_path = os.path.join("../data", f"dataCD_{num_knots}_01.pkl")

if gen_data:
    data = generate_data(scale_list, distribution_list, n_list, lim_list, sparse_list, sigma_list, replication_list, num_knots, data_dict, curve_dict)
