...
def generate_data_dict(distribution_list, n_list, replication_list, x_range=[0.0, 1.0], seed=0):
    ...
    for dist in distribution_list:
        ...
        if dist == 'uniform':
            ...
            bimode = np.array([8.0, 12.0, 16.0])
            second_mode = np.tile(bimode, len(replication_list)//len(bimode) + 1)[:len(replication_list)]
            rows = []
            for r in replication_list:
                x_r = np.concatenate([np.random.normal(0.0, 1.0, n_max//2), np.random.normal(second_mode[r], 1.0, n_max//2)], axis=0)
                rows.append(x_r)
            x = np.array(rows)
...
data_dict_path = os.path.join("../data", "data_dictQ_01_alt_bi.pkl")
if gen_data_dict:
...
curve_dict_path = os.path.join("../data", f"curve_dict_{num_knots}_01_alt_bi.pkl")
...
data_path = os.path.join("../data", f"dataQ_{num_knots}_01_alt_bi.pkl")
