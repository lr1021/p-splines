...
def build_model_ortho_diag(x_data, y_data, a, b, spline_degree, n_internal_knots, implementation, penalised, order, tau_val=0.1000000001):
    model = pm.Model()
    with model:
        # Priors
        beta_0 = pm.Normal('beta_0', mu=0, sigma=100)
        ...
        if implementation=='post_centring': # standard = full B splines, unidentifiable, use for posterior centring evaluation
        ...
        elif implementation=='w_centring': # svd = svd decomposition, decompose f_ construction
            X = B.copy()
            X_plot = B_plot.copy()
            ...
            model.add_coord("w_wc_dim", range(k-1))
            if not penalised:
                w_wc = pm.Normal("w_wc", mu=0, tau=tau, shape=k-1, dims="w_wc_dim")
            else:
                K = A_Q.T @ difference_matrix(k, order=order).T @ difference_matrix(k, order=order) @ A_Q
                K_V, K_U = np.linalg.eigh(K)
                Q = tau_p * K_V + tau
                # Q = pt.maximum(Q, 1e-8)
                w_wc = pm.Normal("w_wc", mu=0, tau=Q, shape=k-1, dims="w_wc_dim")
                X = X @ K_U
                X_plot = X_plot @ K_U
            # return K, K_V, K_U, Q, X, X_plot
            f = pm.math.dot(X, w_wc)
            var_names += ['w_wc']

        elif implementation=='ortho_w_centring': # svd = svd decomposition, decompose f_ construction
            X = B.copy()
            X_plot = B_plot.copy()
            ...
            model.add_coord("w_wc_ortho_dim", range(k-1))
            if not penalised:
                w_wc_ortho = pm.Normal("w_wc_ortho", mu=0, tau=tau, shape=k-1, dims="w_wc_ortho_dim")
            else:
                K = Vt @ A_Q.T @ difference_matrix(k, order=order).T @ difference_matrix(k, order=order) @ A_Q @ Vt.T
                K_V, K_U = np.linalg.eigh(K)
                Q = tau_p * K_V + tau
                # Q = pt.maximum(Q, 1e-8)
                w_wc_ortho = pm.Normal("w_wc_ortho", mu=0, tau=Q, shape=k-1, dims="w_wc_ortho_dim")
                X = X @ K_U
                X_plot = X_plot @ K_U
            # return K, K_V, K_U, Q, X, X_plot
            f = pm.math.dot(X, w_wc_ortho)
            var_names += ['w_wc_ortho']
...
def build_model_ortho_diag_softplus(x_data, y_data, a, b, spline_degree, n_internal_knots, implementation, penalised, order, tau_val=1e-2):
    model = pm.Model()
    with model:
        # Priors
        beta_0 = pm.Normal('beta_0', mu=0, sigma=100)
        ...
        eta = beta_0 + f
        # Likelihood
        y_obs = pm.Normal('y_obs', mu=softplus(eta), sigma=np.sqrt(sigma_2), observed=y_data)
    return model, X, X_plot, var_names
...
builder_dict = {'0': build_model,
                ...
                'ortho_diag_softplus': build_model_ortho_diag_softplus,
